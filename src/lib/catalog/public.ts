...
import { mapSectionRow } from '@/lib/cms/repository';
import type { CmsSection, SectionType } from '@/lib/cms/sections';
import { createServerSupabaseClient } from '@/lib/supabase/server';

export interface PublicGame {
  id: string; name: string; slug: string; shortDescription: string; description: string;
  logoUrl: string | undefined; imageUrl: string | undefined; bannerUrl: string | undefined;
  sections: CmsSection<SectionType>[];
}
type GameRow = { id:string; name:string; slug:string; short_description:string|null; description:unknown; logo_media_id:string|null; image_media_id:string|null; banner_media_id:string|null };
...
async function mediaUrlMap(ids:string[]):Promise<Map<string,string>>{ const unique=[...new Set(ids.filter(Boolean))]; const urls=new Map<string,string>(); if(!unique.length)return urls; const supabase=createServerSupabaseClient(); const {data,error}=await supabase.from('media_assets').select('id,storage_bucket,storage_path').in('id',unique).is('deleted_at',null); if(error)throw new AppError('INTERNAL_ERROR','Unable to load catalog media',{code:error.code}); for(const asset of data??[])urls.set(asset.id,supabase.storage.from(asset.storage_bucket).getPublicUrl(asset.storage_path).data.publicUrl); return urls; }
